...
| helm | v3.20.2 | k8s 1.33 지원 |
| nerdctl | v2.2.2 (full) | rootlesskit, slirp4netns 포함 |
| **CNI 옵션 A** | Calico v3.31 (`calico.yaml` 또는 Tigera Operator) | kube-proxy 사용, L7은 `envoy-1.37.2/` 독립 설치 파일 활용 |
| **CNI 옵션 B** | Cilium v1.19.3 | `kubeProxyReplacement=true`, `cilium-1.19.3/` 독립 설치 파일 활용 |
| **L7 Gateway** | Envoy v1.37.2 (Envoy Gateway) | CNI 옵션 A 선택 시 활용, `envoy-1.37.2/` 독립 설치 파일 |

## 2. 필수 시스템 컨테이너 이미지

`kubeadm` 초기화 및 CNI 배포 시 생성되는 필수 컨테이너입니다. 폐쇄망에서는 `download_assets_offline.sh`로 사전에 `.tar` 파일로 확보합니다.
...
│   └── utils/                 # calico.yaml, tigera-operator.yaml, calico-custom-resources.yaml, local-path-storage.yaml
└── scripts/
    ├── download_assets_offline.sh            # [인터넷 호스트] 파일 수집
...
[폐쇄망 노드 / WSL2]
  ...
  scripts/install.sh
    ...
    └─ CNI=calico → manifest/operator 방식 선택 → ../envoy-1.37.2/scripts/install.sh 자동 호출
    └─ CNI=cilium → ../cilium-1.19.3/scripts/install.sh 자동 호출 (독립 설치 파일 활용)
...
| 경로 | 용도 | 호출 시점 |
| --- | --- | --- |
| `../cilium-1.19.3/scripts/install.sh` | Cilium CNI — 독립 설치 파일 활용 | `CNI_CHOICE=cilium` 선택 시 자동 |
