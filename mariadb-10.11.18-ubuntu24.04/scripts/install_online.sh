...
환경 변수:
  MARIADB_REPO_URL
      MariaDB 10.11.18 Ubuntu 저장소 URL을 재정의합니다.

정확한 버전의 DEB와 의존성을 로컬 폴더에 받은 뒤 오프라인 설치
스크립트를 호출합니다.
EOF
}
...
printf '[INFO] MariaDB 10.11.18 온라인 설치용 DEB를 수집합니다.\n'
"${DOWNLOAD_SCRIPT}"
...
printf '[INFO] 검증된 로컬 DEB로 %s 작업을 시작합니다.\n' "${ACTION}"
