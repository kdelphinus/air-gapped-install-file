# Installation Guide - Apache Kafka v4.0.0 (KRaft Mode)
...
2. 성공적으로 완료되면 아래 파일들이 저장됩니다.
  - `charts/kafka` (Bitnami Kafka 32.4.3 차트)
...
4. 설치가 완료되면 `install.conf` 에 설정 값이 저장되며, 생성되었던 임시 배포 파일인 `values-temp.yaml` 은 보안 및 정갈성을 위해 즉시 자동 소멸됩니다.
...
1. 템플릿 복제본을 만듭니다.
  ...
  ```bash
  cp values.yaml values-temp.yaml
  ```

2. `values-temp.yaml` 을 환경에 맞춰 수동 편집합니다.
  - 이미지 주소 및 태그 수정
  - `broker.replicaCount: 3` 지정
  - 공유 스토리지 사용 시 `broker.podAntiAffinityPreset: soft` 로 지정하여 분산 배치 설정.
  - `hostpath` 단일 노드 테스트 시 `broker.podAntiAffinityPreset: ""` 및 `global.nodeSelector` 를 타겟 노드로 지정.
