...
# 2. 이미지 및 리포지토리 교체 (Harbor 사용 시)
if [ "$IMAGE_SOURCE" == "harbor" ]; then
    HARBOR_PREFIX="${HARBOR_REGISTRY}/${HARBOR_PROJECT}"
    echo "   → Harbor 이미지 리렉토리로 치환: $HARBOR_PREFIX"

    # 기본 이미지 교체
    sed -i "s|repository: \"jenkins/jenkins\"|repository: \"${HARBOR_PREFIX}/jenkins\"|g" ./values.yaml
    sed -i "s|repository: \"jenkins/inbound-agent\"|repository: \"${HARBOR_PREFIX}/inbound-agent\"|g" ./values.yaml
    sed -i "s|repository: kiwigrid/k8s-sidecar|repository: ${HARBOR_PREFIX}/k8s-sidecar|g" ./values.yaml
fi

# 3. 커스텀 이미지로 최종 오버라이드
if [ "$USE_CUSTOM_IMAGE" == "true" ]; then
    echo "   → OpenTofu 커스텀 이미지 설정을 values.yaml에 적용..."
    if [ "$IMAGE_SOURCE" == "harbor" ]; then
        sed -i "s|repository: \"${HARBOR_PREFIX}/jenkins\"|repository: \"${HARBOR_PREFIX}/cmp-jenkins-full\"|g" ./values.yaml
    else
        sed -i "s|repository: \"jenkins/jenkins\"|repository: \"cmp-jenkins-full\"|g" ./values.yaml
    fi
    # tag도 custom 이미지 태그(2.555.3)로 고정
