...
> **Security warning:** This builder is for asset collection and reproducibility checks. Generated bundle installation is disabled because the full security baseline is not implemented. Use a version- and OS-specific secure package for cluster installation.

## 1. 전제 조건

- 온라인 Ubuntu 24.04 또는 Rocky Linux 9 호스트
- root 또는 sudo 권한
- 인터넷 접근 가능
- `curl`, `tar`, `bash`, `apt-get` 사용 가능

> Rocky/RHEL 계열은 현재 `rocky9`을 1차 검증 대상으로 지원합니다.
...
| `CONTAINERD_VERSION` | containerd 버전 정책. Ubuntu는 `auto`, Rocky 9은 `auto` 입력 시 `2.1.*`로 정규화 |
| `CNI_CHOICE` | CNI 선택값 |
...
Ubuntu 24.04는 `/etc/default/kubelet`, Rocky Linux 9은 `/etc/sysconfig/kubelet`에 기록합니다.
...
- containerd 패키지 수집. Rocky 9은 Kubernetes 1.33 호환성 기준으로 `containerd.io-2.1.*` 라인을 사용합니다.
- kubeadm 기준 core image 목록 생성 및 export
- CNI 매니페스트, Helm chart, 이미지 수집
- 번들 생성용 staging 디렉터리 구성

현재 구현은 Ubuntu 24.04와 Rocky Linux 9 기준으로 실제 수집을 수행합니다. 외부 네트워크와 APT/DNF repo 등록이 필요하므로 `sudo`로 실행합니다.

### 설정값 검증

`scripts/download_assets_offline.sh`와 `scripts/build_bundle.sh`는 공통 함수 파일 `scripts/lib/common.sh`를 통해 다음 항목을 먼저 검증합니다.

- `K8S_VERSION`: `v1.33.13` 형식. `1.33.13`처럼 `v`를 생략하면 자동으로 `v1.33.13`로 보정합니다.
- `TARGET_OS`: 현재 `ubuntu24.04` 또는 `rocky9`을 허용합니다.
...
```text
bundles/k8s-v1.33.13-ubuntu24.04/
bundles/k8s-v1.33.13-ubuntu24.04.tar.gz
bundles/k8s-v1.33.13-rocky9/
bundles/k8s-v1.33.13-rocky9.tar.gz
```
...
1. Rocky 9 실환경 RPM 수집/설치 스모크 테스트
2. 기존 `k8s-1.33.13-ubuntu24.04` 산출물 재현 검증
