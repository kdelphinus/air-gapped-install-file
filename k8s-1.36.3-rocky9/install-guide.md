# Kubernetes v1.36.3 Rocky 9 보안 설치 가이드

## 1. 적용 대상

이 가이드는 Rocky Linux 9 amd64 신규 노드에 Kubernetes `v1.36.3`을 설치할 때 사용합니다. 작성 시점의 최신 Rocky 9 마이너는 `9.8`이며, 수집 스크립트는 실행 시점의 최신 Rocky 9 마이너로 호스트를 업데이트합니다.
...
- Kubernetes `v1.36.3` RPM 및 전체 의존 RPM
...
sudo kubeadm config validate --config /run/kubeadm-v1.36.3.yaml
