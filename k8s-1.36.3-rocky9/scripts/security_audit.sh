...
check_rocky_host() {
    local os_id os_version source_cid
    ...
    if [ "$os_id" = "rocky" ] && [[ "$os_version" == 9.* ]]; then
        pass "운영체제 Rocky Linux ${os_version}"
    else
        fail "지원 운영체제가 아님: ${os_id:-unknown} ${os_version:-unknown}"
    fi

    if command -v getenforce >/dev/null 2>&1 && [ "$(getenforce)" = "Enforcing" ]; then
        pass "SELinux Enforcing"
    else
        fail "SELinux가 Enforcing이 아님"
    fi
    ...
    if grep -Eq '^[[:space:]]*SystemdCgroup[[:space:]]*=[[:space:]]*true' \
       ...
       grep -Eq '^[[:space:]]*enable_selinux[[:space:]]*=[[:space:]]*true' \
        ...
        pass "containerd SystemdCgroup 및 SELinux 활성화"
    else
        fail "containerd SystemdCgroup 또는 SELinux 설정 불완전"
    fi

    if [ -f /etc/NetworkManager/conf.d/99-calico.conf ]; then
        pass "NetworkManager Calico 인터페이스 예외"
    else
        fail "NetworkManager Calico 인터페이스 예외 파일 없음"
    fi
    ...
    if [ -f /etc/kubernetes/installer/firewalld-sources ]; then
        while IFS= read -r source_cidr; do
            [ -n "$source_cidr" ] || continue
            if firewall-cmd --permanent --zone=trusted --query-source="$source_cidr" \
                >/dev/null 2>&1; then
                pass "firewalld trusted source 등록: ${source_cidr}"
            else
                fail "firewalld trusted source 누락: ${source_cidr}"
            fi
        done < /etc/kubernetes/installer/firewalld-sources
    else
        fail "firewalld source 기록 파일 없음"
    fi
}
main() {
    check_root
