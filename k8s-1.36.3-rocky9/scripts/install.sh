...
prepare_security_assets() {
    ...
    install -d -m 0755 /etc/kubernetes /etc/kubernetes/pki \
    ...
    for selinux_path in "${selinux_paths[@]}"; do
        semanage fcontext -a -t container_file_t "$selinux_path"
    done
    restorecon -RF /etc/kubernetes /var/lib/etcd /var/log/kubernetes
