...
# 버전 설정
K8S_VERSION="v1.33.7"
K8S_RPM_VERSION="1.33.7"
K8S_DEB_VERSION="1.33.7-1.1"
...
if [ "$DL_BINS" = true ]; then
    ...
    # Calico 매니페스트 (operator, custom-resources 포함)
    ...
    curl -fsSL "https://raw.githubusercontent.com/projectcalico/calico/${CALICO_VERSION}/manifests/tigera-operator.yaml" -o "${UTIL_DIR}/tigera-operator.yaml"
    curl -fsSL "https://raw.githubusercontent.com/projectcalico/calico/${CALICO_VERSION}/manifests/custom-resources.yaml" -o "${UTIL_DIR}/calico-custom-resources.yaml"
