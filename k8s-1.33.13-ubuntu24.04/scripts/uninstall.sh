...
UFW_MARKER="/etc/kubernetes/installer/ufw-rules"
RESET=false
...
echo "  - kubeadm reset"
echo "  - /etc/kubernetes 및 런타임 보안 키 삭제"
echo "  - /var/lib/etcd, /var/lib/kubelet, CNI 상태 삭제"
echo "  - 이 노드가 단일 Control Plane이면 클러스터 복구가 불가능할 수 있음"
echo "  - containerd 이미지와 호스트 방화벽 규칙은 보존"
[ "$PURGE_PACKAGES" = "true" ] && echo "  - kubeadm/kubelet/kubectl/containerd 패키지도 제거"
echo ""
...
if [ -f "$UFW_MARKER" ]; then
    mapfile -t ufw_rules < "$UFW_MARKER"
fi
...
echo -e "${CYAN}[2/6] 설치기가 추가한 UFW 규칙 삭제...${NC}"
if command -v ufw >/dev/null 2>&1; then
    for rule in "${ufw_rules[@]}"; do
        ...
        [ -n "$cidr" ] || continue
        case "$kind" in
            ALLOW) ufw --force delete allow from "$cidr" >/dev/null 2>&1 || true ;;
            ROUTE) ufw --force route delete allow from "$cidr" >/dev/null 2>&1 || true ;;
        esac
    done
fi

echo -e "${CYAN}[3/6] Kubernetes 및 CNI 상태 삭제...${NC}"rm -rf /etc/cni/net.d
rm -rf /var/lib/cni
...
if [ "$PURGE_PACKAGES" = "true" ]; then
    echo -e "${CYAN}[6/6] Kubernetes 패키지 제거...${NC}"
    apt-mark unhold kubelet kubeadm kubectl >/dev/null 2>&1 || true
    apt-get remove -y --purge kubeadm kubelet kubectl cri-tools containerd.io
    apt-get autoremove -y
else
    echo -e "${CYAN}[6/6] 패키지 보존${NC}"
fi

echo -e "${GREEN}초기화가 완료되었습니다.${NC}"
echo "UFW 서비스는 유지했으며 설치기가 추가한 CIDR 규칙만 제거했습니다."
