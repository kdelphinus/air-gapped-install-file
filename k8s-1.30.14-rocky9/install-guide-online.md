# Kubernetes v1.30.14 온라인 설치 가이드 (Rocky Linux 9)


> **Security warning:** This historical online workflow is not the supported installation path. Use `scripts/download_assets_offline.sh` and the hardened `scripts/install.sh`.
인터넷이 가능한 환경에서 kubeadm 기반 Kubernetes v1.30.14 클러스터를 구성하는 절차를 안내합니다.
