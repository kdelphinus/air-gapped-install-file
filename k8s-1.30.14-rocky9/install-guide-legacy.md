# Kubernetes v1.30.14 오프라인 설치 가이드 (Rocky Linux 9)


> **Do not use for installation:** This document preserves the pre-hardening workflow for comparison only. Follow `install-guide.md`.
폐쇄망 환경에서 kubeadm 기반 Kubernetes v1.30.14 클러스터를 구성하는 절차를 안내합니다.
...
- [ ] 폐쇄망 반입 자산 확인: RPM, 바이너리, 이미지, 매니페스트, Helm chart, Harbor 인증서와 레지스트리 접속 정보
...
- 단, 본 v1.30.14 설치 가이드의 본문 절차는 아직 Rocky Linux 9 기준입니다. Ubuntu 설치 절차는 별도 검증 후 반영해야 합니다.
