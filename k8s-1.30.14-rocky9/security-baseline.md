# Kubernetes v1.30.14 Rocky 9 보안 기준과 잔여 위험

## 1. 기준

이 문서는 `K8s취약점_점검_가이드.pdf`의 Control Plane 17개 및 Worker 7개 점검 영역을 신규 설치 기본값에 반영한 결과를 설명합니다. 제품 버전은 Kubernetes `v1.30.14`, kubeadm 설정 API는 `v1beta4`입니다.
...
`v1.30.14`과 Rocky 9 최신 마이너는 패키지 작성 시점 기준입니다. 이후 Kubernetes patch release, Rocky errata 또는 보안 공지가 나오면 외부망에서 자산을 다시 수집하고 단계적으로 업그레이드해야 합니다.
