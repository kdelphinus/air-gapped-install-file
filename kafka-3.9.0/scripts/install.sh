#!/bin/bash
# ---------------------------------------------------------
# Apache Kafka v3.9.0 (KRaft Mode) Offline Installation Script
...
# YAML 동기화 및 보존을 위해 임시 파일 삭제 trap 제거
...
save_conf() {
    cat > "$CONF_FILE" <<EOF
# Kafka 3.9.0 설치 설정 — install.sh 에 의해 자동 관리됩니다.
...
cleanup_resources() {
    ...
    if [ "$RESET_MODE" == "reset" ]; then
        rm -f "$CONF_FILE"
        rm -f "./values-custom.yaml"
        echo -e "🗑️  설정 파일 및 커스텀 values 파일 삭제 완료 (Reset)."
    fi
...
# ==========================================
echo ""
echo "🔧 커스텀 설정 파일(values-custom.yaml) 생성 및 치환 중..."

# 1. 템플릿 복사 분기
if [ "${IMAGE_SOURCE}" = "local" ]; then
    cp -f ./values-local.yaml ./values-custom.yaml
else
    cp -f ./values.yaml ./values-custom.yaml
...
# 스토리지 세부 분기에 따른 values-custom.yaml 덧붙이기
if [ "$STORAGE_TYPE" == "nfs-dynamic" ]; then
    # Dynamic SC 인 경우 storageClass 덮어쓰기 및 Anti-Affinity 활성화
    sed -i "s|storageClass: \"manual\"|storageClass: \"${STORAGE_CLASS}\"|g" ./values-custom.yaml
...
echo ""
echo "========================================================"
echo -e "🎉 구성 완료! (Kafka v3.9.0 / KRaft 3-Nodes HA)"
echo "설정 파일 : $CONF_FILE"
echo "Values 파일: ./values-custom.yaml"
