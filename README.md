...
| `monitoring-62.0.0/` | kube-prometheus-stack | 62.0.0 | Prometheus + Grafana + Alertmanager |
...
[인프라 레이어]
4.  harbor-1.14.3        — 내부 레지스트리 (이후 모든 이미지는 Harbor에서 조달)
5.  nfs-provisioner      — StorageClass 확보
6.  metallb              — LoadBalancer IP 풀 (L2 모드)
7.  ingress-nginx / envoy — Ingress / Gateway API
...
harbor, ingress-nginx, metallb, monitoring, envoy, nexus, gitlab, jenkins, argocd, velero는
아래의 표준 구조를 따릅니다.
