...
echo "==========================================="
echo " Uninstalling OpenTelemetry Collector v0.158.0"
echo "==========================================="
read -p "❓ 정말 삭제하시겠습니까? (y/n): " CONFIRM
[[ "$CONFIRM" =~ ^[Yy]$ ]] || { echo "취소되었습니다."; exit 0; }

# Helm 제거
if helm status otel-collector -n "$TARGET_NAMESPACE" > /dev/null 2>&1; then
    echo "🗑️  Helm Release 'otel-collector' 삭제 중..."
    helm uninstall otel-collector -n "$TARGET_NAMESPACE" --wait=false
else
    echo "  - 삭제할 Helm Release 'otel-collector'가 없습니다."
fi
...
for KIND in daemonset deployment service serviceaccount configmap; do
    kubectl get $KIND -n "$TARGET_NAMESPACE" -o name 2>/dev/null | grep otel-collector | \
...
echo ""
echo "✅ OpenTelemetry Collector 삭제 완료."
