...
if [ "$EXEC_MODE" == "2" ]; then
    ...
    read -p "Harbor 사용자 [기본값 admin]: " INPUT_USER
    HARBOR_USER="${INPUT_USER:-$HARBOR_USER}"
...
if [ "${USE_PLAIN_HTTP:-false}" = "true" ]; then
...
for tar_file in "${image_archives[@]}"; do
    ...
    while read -r source_image; do
        ...
        if ! ctr -n "$CTR_NAMESPACE" images convert --platform "$TARGET_PLATFORM" "$source_image" "$target_image" > /dev/null 2>&1; then
