...
- `SAVE_PATH` (데이터 저장 경로)는 `NODE_NAME` 노드에서 디렉토리가 생성되어 있어야 함 (권한: `chmod 777`)

## Phase 1: 이미지 업로드

Harbor 설치에 필요한 컨테이너 이미지를 노드에 로드합니다.
모든 워커 노드에서 실행해야 합니다.
...
```bash
ctr -n k8s.io images list | grep harbor
```

## Phase 2: Harbor 설치 스크립트 설정

`harbor_install_offline.sh` 상단 Config 블록을 환경에 맞게 수정합니다.

| 변수 | 설명 | 예시 |
| :--- | :--- | :--- |
| `HARBOR_NAMESPACE` | Harbor 설치 네임스페이스 | `harbor` |
| `HARBOR_RELEASE_NAME` | Helm release 이름 | `harbor` |
| `HELM_CHART_PATH` | Helm 차트 파일 경로 | `./harbor-1.14.3.tgz` |
...
## Phase 3: Harbor 설치 실행

```bash
chmod +x harbor_install_offline.sh
...
기존 설치가 감지되면 삭제 후 재설치 여부를 확인합니다.

## Phase 4: (TLS 미사용 시) Insecure Registry 등록

TLS를 사용하지 않는 경우, 모든 노드에서 containerd에 insecure registry를 등록합니다.

```bash
chmod +x insecurity_registry_add.sh
./insecurity_registry_add.sh
```

스크립트 실행 시 Harbor 주소 (`<NODE_IP>:30002`)를 입력합니다.

## Phase 5: (선택) Self-Signed TLS 인증서 생성

도메인 접속 시 자체 서명 인증서가 필요한 경우 생성합니다.

```bash
chmod +x create_self-signed_tls.sh
./create_self-signed_tls.sh
```
