...
본 컴포넌트는 단일 Kafka/Zookeeper 아키텍처를 대체하기 위해 구성된
**Redis Stream 7.2.4 (Master-Replica-Sentinel HA)** 클러스터입니다.

## 주요 특징

- **고가용성**: Master 1 + Replica 2 + Sentinel 3 구성을 통한 자동 Failover 지원
- **환경 최적화**: 운영(Harbor) 및 로컬(ctr import) 환경에 최적화된 설치 스크립트 제공
...
> **참고**: 로컬 환경은 Master 1 + Replica 1 + Sentinel 2 구성으로 축소 배포됩니다.
> 운영 환경은 Master 1 + Replica 2 + Sentinel 3 구성입니다.

## 접속 정보 및 포트

- **Sentinel 엔드포인트**: `redis-stream.redis-stream.svc:26379`
- **Master Set 이름**: `mymaster`
- **포트**: 6379 (Redis), 26379 (Sentinel)
